...
func ParseRoster(reply Stanza) ([]RosterEntry, error) {
	...
	if !ok {
		return nil, errors.New(
			"xmpp: roster request resulted in tag of type " + reply.Name.Local,
		)
	}
...
func (c *Conn) SendIQ(
	to, typ string,
	value interface{},
) (reply chan Stanza, cookie Cookie, err error) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Conn) Send(to, msg string) error {
	...
	_, err := fmt.Fprintf(
		c.out,
		"<message to='%s' from='%s' type='chat'><body>%s</body>%s</message>",
		xmlEscape(to),
		xmlEscape(c.jid),
		xmlEscape(msg),
		archive,
...
func (c *Conn) getFeatures(domain string) (features streamFeatures, err error) {
	...
	if se.Name.Space != NsStream || se.Name.Local != "stream" {
		err = errors.New(
			"xmpp: expected <stream> but got <" + se.Name.Local + "> in " + se.Name.Space,
		)
		return
	}
...
// given user.
func Dial(
	address, user, domain, resource, password string,
	config *Config,
) (c *Conn, err error) {
	c = new(Conn)
	...
	if !config.SkipTLS {
		...
		if proceed.Name.Space != NsTLS || proceed.Name.Local != "proceed" {
			return nil, errors.New(
				"xmpp: expected <proceed> after <starttls> but got <" + proceed.Name.Local + "> in " + proceed.Name.Space,
			)
		}
		...
		/*
			if haveCertHash {
				...
				if digest := h.Sum(nil); !bytes.Equal(
					digest,
					config.ServerCertificateSHA256,
				) {
					return nil, fmt.Errorf(
						"xmpp: server certificate does not match expected hash (got: %x, want: %x)",
						digest,
						config.ServerCertificateSHA256,
					)
				}
	...
	*/
	...
	if config != nil && config.CreateCallback != nil {
		...
		if len(register.Form.Type) > 0 {
			...
			fmt.Fprintf(
				c.rawOut,
				"<iq type='set' id='create_2'><query xmlns='jabber:iq:register'>",
			)
	...
	if len(resource) == 0 {
		...
		fmt.Fprintf(
			c.out,
			"<iq type='set' id='bind_1'><bind xmlns='%s'/></iq>",
			NsBind,
		)
	} else {
		fmt.Fprintf(c.out,
	...
	if features.Session != nil {
		...
		fmt.Fprintf(
			c.out,
			"<iq to='%s' type='set' id='sess_1'><session xmlns='%s'/></iq>",
			domain,
			NsSession,
		)
		if err = c.in.DecodeElement(&iq, nil); err != nil {
...
var defaultStorage = map[xml.Name]reflect.Type{
	xml.Name{Space: NsStream, Local: "features"}: reflect.TypeOf(
		streamFeatures{},
	),
	...
	xml.Name{Space: NsSASL, Local: "mechanisms"}: reflect.TypeOf(
		saslMechanisms{},
	),
	...
	xml.Name{Space: NsClient, Local: "message"}: reflect.TypeOf(
		ClientMessage{},
	),
	xml.Name{Space: NsClient, Local: "presence"}: reflect.TypeOf(
		ClientPresence{},
	),
...
// VerificationString returns a SHA-1 verification string as defined in
// XEP-0115.
...
func (s *xep0115Sorter) Less(
	i, j int,
) bool {
	return s.s[i].xep0115Less(s.s[j])
}
