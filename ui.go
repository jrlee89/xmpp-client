...
import (
	...
	"github.com/jrlee89/xmpp-client/xmpp"
...
var createAccount *bool = flag.Bool(
	"create",
	false,
...
var OTRWhitespaceTagStart = []byte(
...
func terminalMessage(
	...
	line = append(
		line,
		[]byte(fmt.Sprintf(" (%s) ", time.Now().Format(time.Kitchen)))...)
	if critical {
...
func main() {
	...
	if len(*configFile) == 0 {
		...
		if len(homeDir) == 0 {
			alert(
				term,
				"$HOME not set. Please either export $HOME or use the -config-file option.\n",
			)
			return
		}
	...
	for i := len(config.Proxies) - 1; i >= 0; i-- {
		...
		if err != nil {
			alert(
				term,
				"Failed to parse "+config.Proxies[i]+" as a URL: "+err.Error(),
			)
			return
		}
		if dialer == nil {
		...
		if dialer, err = proxy.FromURL(u, dialer); err != nil {
			alert(
				term,
				"Failed to parse "+config.Proxies[i]+" as a proxy: "+err.Error(),
			)
			return
		}
	}
	...
	if len(config.ServerCertificateSHA256) > 0 {
		...
		if err != nil {
			alert(
				term,
				"Failed to parse ServerCertificateSHA256 (should be hex string): "+err.Error(),
			)
			return
		}
	...
	if domain == "jabber.ccc.de" {
		...
		if err == nil {
			alert(
				term,
				"Temporarily trusting only CACert root for CCC Jabber server",
			)
	...
	info(
		term,
		fmt.Sprintf("Your fingerprint is %x", s.privateKey.Fingerprint()),
	)
...
func (s *Session) processIQ(stanza *xmpp.ClientIQ) interface{} {
	...
	case "jabber:iq:roster query":
		...
		if err := xml.NewDecoder(bytes.NewBuffer(stanza.Query)).Decode(&roster); err != nil ||
			len(roster.Item) == 0 {
	...
	default:
		info(
			s.term,
			"Unknown IQ: "+startElem.Name.Space+" "+startElem.Name.Local,
		)
	}
...
func (s *Session) ignoreUser(uid string) {
	...
	if hasContact {
		info(
			s.input.term,
			fmt.Sprintf(
				"Ignoring messages from %s for the duration of this session",
				uid,
			),
		)
	} else {
	...
	info(
		s.input.term,
		fmt.Sprintf(
			"Use '/unignore %s' to continue receiving messages from them.",
			uid,
		),
	)
}
...
func (s *Session) ignoreList() {
	...
	info(
		s.input.term,
		"Ignoring messages from these users for the duration of the session:",
	)
...
func (s *Session) processClientMessage(stanza *xmpp.ClientMessage) {
	...
	out, encrypted, change, toSend, err := conversation.Receive(
		[]byte(stanza.Body),
	)
	if err != nil {
		...
		if s.config.OTRAutoTearDown {
			if s.conversations[from] == nil {
				alert(
					s.term,
					fmt.Sprintf(
						"No secure session established; unable to automatically tear down OTR conversation with %s.",
						from,
					),
				)
				break
	...
	case otr.SMPSecretNeeded:
		info(
			s.term,
			fmt.Sprintf(
				"%s is attempting to authenticate. Please supply mutual shared secret with /otr-auth user secret",
				from,
			),
		)
	...
	case otr.SMPComplete:
		...
		if len(s.config.UserIdForFingerprint(fpr)) == 0 {
			s.config.KnownFingerprints = append(
				s.config.KnownFingerprints,
				KnownFingerprint{fingerprint: fpr, UserId: from},
			)
		}
		s.config.Save()
	case otr.SMPFailed:
	...
	if !encrypted && len(out) >= whitespaceTagLength {
		...
		if bytes.Equal(
			whitespaceTag[:len(OTRWhitespaceTagStart)],
			OTRWhitespaceTagStart,
		) {
			if bytes.HasSuffix(whitespaceTag, OTRWhiteSpaceTagV1) {
				info(
					s.term,
					fmt.Sprintf(
						"%s appears to support OTRv1. You should encourage them to upgrade their OTR client!",
						from,
					),
				)
	...
	if s.config.OTRAutoStartSession && detectedOTRVersion >= 2 {
		info(
			s.term,
			fmt.Sprintf(
				"%s appears to support OTRv%d. We are attempting to start an OTR session with them.",
				from,
				detectedOTRVersion,
			),
		)
	...
	if stanza.Delay != nil && len(stanza.Delay.Stamp) > 0 {
		...
		if err != nil {
			alert(
				s.term,
				"Can not parse Delayed Delivery timestamp, using quoted string instead.",
			)
...
func (s *Session) maybeNotify() {
	...
	notifyTime := s.lastActionTime.Add(
		time.Duration(idleThreshold) * time.Second,
	)
	...
	cmd := exec.Command(
		s.config.NotifyCommand[0],
		s.config.NotifyCommand[1:]...)
	go func() {
...
func (s *Session) processPresence(stanza *xmpp.ClientPresence) {
	...
	case "subscribe":
		...
		info(
			s.term,
			jid+" wishes to see when you're online. Use '/confirm "+jid+"' to confirm (or likewise with /deny to decline)",
		)
...
func (s *Session) awaitVersionReply(ch <-chan xmpp.Stanza, user string) {
	...
	if err := xml.NewDecoder(buf).Decode(&versionReply); err != nil {
		warn(
			s.term,
			"Failed to parse version reply from "+user+": "+err.Error(),
		)
		return
	}
...
func unescapeNonASCII(in string) (string, error) {
	...
	for i := 0; i < len(in); i++ {
		if in[i] == '\\' {
			...
			if in[i+1] != 'x' {
				return "", errors.New(
					"escape sequence didn't start with \\x in: " + in,
				)
			}
			v, err := strconv.ParseUint(in[i+2:i+4], 16, 8)
			if err != nil {
				return "", errors.New(
					"failed to parse value in '" + in + "': " + err.Error(),
				)
			}
...
func (s *Session) processEditedRoster(edit *rosterEdit) bool {
	...
	for i, line := range lines {
		...
		if entry.Jid, err = unescapeNonASCII(string(string(parts[0]))); err != nil {
			alert(
				s.term,
				fmt.Sprintf("Failed to parse JID on line %d: %s", i+1, err),
			)
			return false
		}
		for _, part := range parts[1:] {
			...
			if pos == -1 {
				alert(
					s.term,
					fmt.Sprintf("Failed to find colon in item on line %d", i+1),
				)
				return false
			}
			...
			if err != nil {
				alert(
					s.term,
					fmt.Sprintf(
						"Failed to unescape item on line %d: %s",
						i+1,
						err,
					),
				)
			...
			case "name":
				if len(entry.Name) > 0 {
					alert(
						s.term,
						fmt.Sprintf(
							"Multiple names given for contact on line %d",
							i+1,
						),
					)
					return false
			...
			default:
				alert(
					s.term,
					fmt.Sprintf("Unknown item tag '%s' on line %d", typ, i+1),
				)
				return false
			}
		}
	...
	for _, entry := range edit.roster {
		...
		if newEntry.Name != entry.Name ||
			!setEqual(newEntry.Group, entry.Group) {
...
NextAdd:
	...
	for _, jid := range toDelete {
		...
		newKnownFingerprints := make(
			[]KnownFingerprint,
			0,
			len(s.config.KnownFingerprints),
		)
...
func printConversationInfo(
	...
	info(
		s.term,
		fmt.Sprintf("  Session  ID  for %s: %x", uid, conversation.SSID),
	)
	if fprUid == uid {
...
func promptForForm(
	term *terminal.Terminal,
	user, password, title, instructions string,
	fields []interface{},
) error {
	info(
		term,
		"The server has requested the following information. Text that has come from the server will be shown in red.",
	)
	...
	showMediaEntries := func(questionNumber int, medias [][]xmpp.Media) {
		...
		write(
			"The following media blobs have been provided by the server with this question:\n",
		)
		for i, media := range medias {
			for j, rep := range media {
				...
				write(
					fmt.Sprintf(
						"Data of type %s",
						formStringForPrinting(rep.MIMEType),
					),
				)
				if len(rep.URI) > 0 {
					write(
				...
				if len(tmpDir) == 0 {
					...
					if tmpDir, err = ioutil.TempDir("", "xmppclient"); err != nil {
						write(
							", but failed to create temporary directory in which to save it: " + err.Error() + "\n",
						)
						continue
					}
				}
				...
				filename := filepath.Join(
					tmpDir,
					fmt.Sprintf("%d-%d-%d", questionNumber, i, j),
				)
				if len(fileExt) > 0 {
				...
				if err != nil {
					write(
						", but failed to create file in which to save it: " + err.Error() + "\n",
					)
					continue
				}
				out.Write(rep.Data)
